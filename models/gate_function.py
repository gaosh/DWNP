...
class virtual_gate(nn.Module):
    ...
    def enable_fn(self):
        self.collect_fn = True
        self.count = 0
        self.fn = None

    def calculate_fn(self, input):
        if self.fn is None:
            self.fn = input.pow(2).sum(dim=(2, 3)).mean(0)
        else:
            self.fn += input.pow(2).sum(dim=(2, 3)).mean(0)
        self.count += 1

    def finalize_fn(self):
        self.fn = self.fn / self.count
        self.collect_fn = False

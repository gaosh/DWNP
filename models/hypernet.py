...
def merge_output(static_vector, dynamic_vector):
    pruned_index = torch.nonzero(static_vector < 0.5)

    dv = dynamic_vector.clone()
    if dv.dim() == 1:
        dv[pruned_index] = 1
    elif dv.dim() == 2:
        dv[:,pruned_index] = 1
    dynamic_vector = dv.clone()

    return dynamic_vector

class DynamicEmbedding(nn.Module):
    def __init__(self, structure=None, T=0.4, base=2, num_clients=10):
        ...
        self.embedding = nn.Embedding(num_clients, len(structure)*64)
        ...
        # decay
        self.remain_index = None
        self.base = base

    def forward(self, task_id):
        batch_size = task_id.size(0)
        inputs = self.embedding(task_id)
        # print(inputs.size())
        inputs = inputs.view(batch_size, len(self.structure), -1)
        inputs = inputs.permute(1,0,2)
        h0 = self.h0.repeat(1, batch_size, 1)
        if self.bn1.weight.is_cuda:
            # self.inputs = self.inputs.cuda()
            h0 = h0.cuda()
        outputs, hn = self.Bi_GRU(inputs, h0)
    ...
    def resource_output(self, task_id):
    ...
    def prune_output(self, vector):
        if vector.is_cuda:
            remain_index = self.remain_index.cuda()
        return vector[remain_index]

...
from models.resnet_hyper import Conv2d_ws
import torchvision
...
class Flops_constraint_mobnet(nn.Module):
    def __init__(self, p, kernel_size, out_size, group_size, size_inchannel, size_outchannel, in_channel=3, w=2, HN=False, structure=None):
    ...
    def forward(self, input):
        ...
        if resource_ratio > self.p:
            ...
            # resource_ratio = (sum_flops / self.t_flops)
            abs_rv = torch.clamp(resource_ratio, max=self.p - 0.005)
            loss = self.lower_w*torch.log(((self.p) / abs_rv))
...
class Logger(object):
    def __init__(self, dire, prefix=''):
        ...
        self.property = {
            ...
            'tr_correct_samples': [],
            'tr_hyper_samples': [],
            'tr_hyper_correct_samples': [],
            'tr_hyper_res_dynamic_loss': [],
            'test_samples': [],
        ...
        Path(self.dire).mkdir(parents=True, exist_ok=True)
    def update(self, name, value):   
        self.property[name].append(value)
    
        
    def save(self):
        for key, val in self.property.items():
            if key.endswith('samples') or len(val) == 0: continue
            np.save(self.dire + '/' + key + self.prefix, val)

    def print(self, epoch):
        print('Epoch: %d Train-Loss: %.3f | Train-Acc: %.3f%% (%d/%d) | Hyper-Loss: %.3f Hyper-Res-Loss: %.3f | Hyper-Acc: %.3f%% (%d/%d) | Test-Loss: %.3f | Test-Acc: %.3f%% (%d/%d) | Best Acc: %.3f | Comm-Time %.3f | Total-Time %.3f | Percent %.3f'
             % (epoch, self.property['tr_loss'][-1], self.property['tr_acc'][-1], self.property['tr_correct_samples'][-1], self.property['tr_samples'][-1],
             self.property['tr_hyper_loss'][-1], self.property['tr_hyper_res_loss'][-1], self.property['tr_hyper_acc'][-1], self.property['tr_hyper_correct_samples'][-1], self.property['tr_hyper_samples'][-1],
             self.property['test_loss'][-1], self.property['test_acc'][-1], self.property['test_correct_samples'][-1], self.property['test_samples'][-1], self.property['best_test_acc'][-1],
             self.property['comm_time'][-1], self.property['tr_time'][-1], self.property['comm_percent'][-1]))

    def print_dynamic(self, epoch):
        print('Epoch: %d Train-Loss: %.3f | Train-Acc: %.3f%% (%d/%d) | Hyper-Loss: %.3f Hyper-Res-Loss: %.3f Hyper-Res-Dynamic-Loss: %.3f | Hyper-Acc: %.3f%% (%d/%d) | Test-Loss: %.3f | Test-Acc: %.3f%% (%d/%d) | Best Acc: %.3f | Comm-Time %.3f | Total-Time %.3f | Percent %.3f'
             % (epoch, self.property['tr_loss'][-1], self.property['tr_acc'][-1], self.property['tr_correct_samples'][-1], self.property['tr_samples'][-1],
             self.property['tr_hyper_loss'][-1], self.property['tr_hyper_res_loss'][-1], self.property['tr_hyper_res_dynamic_loss'][-1],
                self.property['tr_hyper_acc'][-1], self.property['tr_hyper_correct_samples'][-1], self.property['tr_hyper_samples'][-1],
             self.property['test_loss'][-1], self.property['test_acc'][-1], self.property['test_correct_samples'][-1], self.property['test_samples'][-1], self.property['best_test_acc'][-1],
             self.property['comm_time'][-1], self.property['tr_time'][-1], self.property['comm_percent'][-1]))

    def print_model(self, epoch):
        print('Epoch: %d Train-Loss: %.3f | Train-Acc: %.3f%% (%d/%d) | Test-Loss: %.3f | Test-Acc: %.3f%% (%d/%d) | Best Acc: %.3f'
             % (epoch, self.property['tr_loss'][-1], self.property['tr_acc'][-1], self.property['tr_correct_samples'][-1], self.property['tr_samples'][-1],
             self.property['test_loss'][-1], self.property['test_acc'][-1], self.property['test_correct_samples'][-1], self.property['test_samples'][-1], self.property['best_test_acc'][-1]))

    def print_model_dynamic(self, epoch):
        print('Epoch: %d Train-Loss: %.3f Hyper-Res-Dynamic-Loss: %.3f | Train-Acc: %.3f%% (%d/%d) | Test-Loss: %.3f | Test-Acc: %.3f%% (%d/%d) | Best Acc: %.3f'
             % (epoch, self.property['tr_loss'][-1], self.property['tr_hyper_res_dynamic_loss'][-1], self.property['tr_acc'][-1], self.property['tr_correct_samples'][-1], self.property['tr_samples'][-1],
             self.property['test_loss'][-1], self.property['test_acc'][-1], self.property['test_correct_samples'][-1], self.property['test_samples'][-1], self.property['best_test_acc'][-1]))
...
def print_model_param_flops(model=None, input_res=224, multiply_adds=False):
    ...
    def foo(net):
        ...
        if not childrens:
            if isinstance(net, torch.nn.Conv2d) or isinstance(net, Conv2d_ws):
